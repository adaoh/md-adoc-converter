...
- `PHIG: real, default: 0`: Angle anti-clockwise from X-axis to largest gradient \f$[\mathrm{deg}]\f$
...
The figure shows the initial stiffness \f$ \mathrm{K_i}\f$ and updated secant stiffness \f$ \mathrm{K_s}\f$ 
...
- `TEMP: real, default: 0`: Temperature at which the specification applies \f$ \mathrm{[Temp]} \f$
- `ALPHA: character/real, default: 0`: Thermal expansion coefficient \f$ \mathrm{\left[ Temp^{-1} \right] } \f$
    - `= STEE`: The value \f$ 1.2 \times 10^{-5} \f$ is used
    - `= TI23`: The value \f$ 9.0 \times 10^{-6} \f$ is used
    - These values are applicable for temperatures in Celcius or Kelvin
- `BETA: character/real, default: 0`: Pressure expansion coefficient \f$ \mathrm{\left[ 1/\left(F/L^2\right) \right] } \f$
    ...
    \f$ \mathrm {\frac{DIAST(1-2 \nu )}{4 THST \times EMOD}} \f$ where \f$ \mathrm {\nu = \frac{EMOD}{2 GMOD}} -1 \f$
...
- `DIAST: real`: Diameter of pipe \f$ \mathrm{ [L] }\f$
    - `DIAST > 0`: Outer diameter of pipe
    - `DIAST < 0`: Inner diameter of pipe
- `THST: real`: Thickness of pipe \f$ \mathrm{ [L] }\f$
- `DENSST: real`: Density of pipe material \f$ \mathrm{  \left[M/L^3 \right] }\f$
- `THEX: real, default: 0`: Thickness of external coating \f$ \mathrm{ [L] }\f$
- `DENSEX: real, default: 0`: Density of external coating \f$ \mathrm{  \left[M/L^3 \right] }\f$
- `R_EXTCNT: real, default: 0`: Outer contact radius \f$ \mathrm{ [L] }\f$
- `R_INTCNT: real, default: 0`: Inner contact radius \f$ \mathrm{ [L] }\f$

Buoyancy is calculated from the total external diameter \f$ \mathrm{DIAST + 2 \times THEX} \f$ (For `DIAST > 0`)
or \f$ \mathrm{|DIAST| + 2 \times THST + 2 \times THEX} \f$ (For `DIAST < 0`).
...
- `GMOD: real > 0`: Shear modulus \f$ \mathrm{ \left[F/L^2 \right]} \f$
- `SIGY: real`: Yield stress \f$ \mathrm{ \left[F/L^2 \right]} \f$
- `EMODY/NPAIR: real/integer`: 
    - `MATKIND = 2`: Slope of strain-stress curve for plastic region \f$ \mathrm{ \left[F/L^2 \right]} \f$.
        - `EMODY` < `EMOD`
...
\f$ \mathrm{ GMOD  \frac { \pi \left( D_e^2 - D_i^2 \right)} {4}  0.5 } \f$
...
- `EPS(i): real`: Strain for point i on strain-stress curve \f$ \mathrm{[1]} \f$
- `SIG(i): real`: Stress for point i on strain-stress curve \f$ \mathrm{ \left[F/L^2 \right]} \f$
...
- `CMPTYP-ID: character(8)`: Component type identifier
- `TEMP: real, default: 0`: Temperature at which the specification applies \f$ \mathrm{[Temp]} \f$
- `ALPHA: real, default: 0`: Thermal expansion coefficient \f$ \mathrm{ \left[Temp^{-1} \right]} \f$
- `BETA: real, default: 0`: Pressure expansion coefficient \f$ \mathrm{ \left[1/\left(F/L^2\right) \right]} \f$
    - `BETA` gives the expansion of an element with zero effective tension from the difference between
    the internal and the external pressure.
...
- `AMS: real`: Mass/unit length \f$ \mathrm{[M/L]} \f$
- `AE: real`: External cross-sectional area \f$ \mathrm{ \left[L^2 \right]} \f$
- `AI: real`: Internal cross-sectional area \f$ \mathrm{ \left[L^2 \right]} \f$
- `RGYR: real`: Radius of gyration about local x-axis \f$ \mathrm{[L]} \f$
- `AST: real`: Cross-section area for stress calculations \f$ \mathrm{ \left[L^2 \right]} \f$
    - The default value is calculated as seen below
- `WST: real`: Cross-section modulus for stress calculations \f$ \mathrm{ \left[L^3 \right]} \f$
    - The default value is calculated as seen below
- `DST: real`: Diameter for stress calculations \f$ \mathrm{[L]} \f$
    - The default value is calculated as seen below
- `THST: real`: Thickness for stress calculations \f$ \mathrm{[L]} \f$
    - The default value is calculated as seen below
- `R_EXTCNT: real, default: 0`: External contact radius \f$ \mathrm{[L]} \f$
- `R_INTCNT: real, default: 0`: Inner contact radius \f$ \mathrm{[L]} \f$
...
- `AST` \f$ \mathrm{= AE - AI} \f$
- `WST` \f$ \mathrm{= \pi \left( D_e^4 - D_i ^4 \right) / \left( 32 D_e \right)} \f$
- `DST` \f$ \mathrm{= D_e} \f$
- `THST` \f$ \mathrm{= \left( D_e - D_i \right) / 2} \f$
- where \f$ \mathrm{D_e = \sqrt{ \frac{4AE}{\pi} } } \f$ and \f$ \mathrm{D_i = \sqrt{ \frac{4AI}{\pi} } } \f$
...
- `IMF` \f$\neq \f$  `0` should be used with care as the analysis can become unstable.
...
- `EA: real > 0`: Axial stiffness \f$ \mathrm{[F]} \f$
...
- `EAF(1): real`: Axial force corresponding to relative elongation `ELONG(1)` \f$ \mathrm{[F]} \f$
...
- `EI: real > 0`: Bending stiffness \f$ \mathrm{ \left[FL^2 \right]} \f$
- `GAs: real`: Shear stiffness \f$ \mathrm{[F]} \f$
...
- `EI: real`: Bending stiffness \f$ \mathrm{\left [FL^2 \right]} \f$
- `MF: real`: Internal friction moment, see figure below. \f$ \mathrm{[FL]} \f$
- `SF: real, default: 10.`: Internal friction moment stiffness factor. \f$ \mathrm{[1]} \f$
...
- `EI(1): real`: Bending stiffness \f$ \mathrm{\left[ FL^2 \right] } \f$
- `PRESS(1): real`: Pressure at which the above values apply \f$ \mathrm{\left[ F/L^2 \right] } \f$
...
For `IMF` \f$ \neq \f$ `0` cfr. [Bending stiffness. Case 4...](@ref inpmod_c_crs1_bending_stiffness_case_4)
...
- `CURV(1): real`: Curvature values for which bending moment is specified \f$ \mathrm{[1/L]} \f$
- .
- .
- .
...
- `BMOMY(1): real`: Bending moment around y-axis \f$ \mathrm{[FL]} \f$ corresponding to curvature values given above in 'Curvature'.
- `BMOMY(N)`
...
- `PRESS(1): real`: Pressure levels for which bending moment is specified \f$ \mathrm{ \left[F/L^2 \right]} \f$
- `PRESS(N)`:
...
- `BMOMY(1,1): real`: Bending moment at curvature I and pressure J \f$ \mathrm{[FL]} \f$.
- `BMOMY(N1,N2)`
...
- `GT-: real > 0`: Torsion stiffness \f$ \mathrm{ \left[FL^2/Radian \right]} \f$
...
- `TMOM(1): real`: Torsion moment \f$ \mathrm{[FL]} \f$
- `TROT(1): real`: Torsion angle/length \f$ \mathrm{[Radian/L]} \f$
- .
- .
- . 
- `TMOM(N)`:
...
\f$ \mathrm{ \boldsymbol{\mathrm{c_m}} = a_{1t}\boldsymbol{\mathrm{m}}_t + a_{1to}\boldsymbol{\mathrm{m}}_{to} + a_{1b}\boldsymbol{\mathrm{m}}_b } \f$

where \f$ \boldsymbol{\mathrm{m}}\f$ is the local stiffness matrix and the subscripts “t”, “to” and “b” refer to axial, torsional and
...
\f$ \mathrm{ \boldsymbol{\mathrm{c_k}} = a_{2t}\boldsymbol{\mathrm{k}}_t + a_{2to}\boldsymbol{\mathrm{k}}_{to} + a_{2b}\boldsymbol{\mathrm{k}}_b } \f$

where \f$ \boldsymbol{\mathrm{k}}\f$ is the local stiffness matrix and the subscripts “t”, “to” and “b” refer to axial, torsional and
bending contributions, respectively.
...
\f$ \mathrm{ F = C(\varepsilon) \times |\dot{\varepsilon}|^P \times sign \left( \dot{\varepsilon} \right) } \f$

where:
- \f$ \mathrm{F} \f$: damping force
- \f$ \mathrm{C} \f$: damping coefficient (strain dependent)
- \f$ \mathrm{\varepsilon} \f$: relative elongation
- \f$ \mathrm{\dot{\varepsilon}} \f$: strain velocity
- \f$ \mathrm{P} \f$: exponent for strain velocity (P >= 1)
...
- `D: real, default:`\f$ \sqrt{\mathrm{\frac{4}{\pi}(AE) }} \f$: Hydrodynamic diameter of the pipe \f$ \mathrm{ [L] } \f$.
...
The tangential force which is a friction force per unit length acting in local x-axis, \f$ \mathrm{Ft} \f$ is computed by:

\f$ \mathrm{Ft = CDX \times VRELX \times |VRELX| + CDLX \times VRELX } \f$

The drag force per unit length acting normal to the local x-axis, \f$ \mathrm{F_n} \f$, is computed by assuming that
the instantaneous drag force direction is parallel to the instantaneous transverse relative velocity component:

\f$ \mathrm{F_n = CDY \left(VRELY^2 + VRELZ^2 \right) + CDLY \times \sqrt{VRELY^2 + VRELZ^2} } \f$


where:
- \f$ \mathrm{CDX, CDY} \f$: are the dimensional quadratic drag force coefficients in local x- and y-directions (i.e. tangential and normal directions)
- \f$ \mathrm{CDLX, CDLY} \f$: are the dimensional linear drag force coefficients in local x- and y-directions
- \f$ \mathrm{VRELX, VRELY, VRELZ} \f$: are relative water velocities in local x,y and z-directions
...
- \f$ \mathrm{CDX = \frac{1}{2} \rho S_W C_{dt}} \f$
- \f$ \mathrm{CDY = \frac{1}{2} \rho D C_{dn}} \f$
- \f$ \mathrm{CDLX = \rho \sqrt{g S_W} \times S_W^2 C^L_{dt}} \f$
- \f$ \mathrm{CDLY = \rho \sqrt{g D} \times D^2 C^L_{dt}} \f$
- \f$ \mathrm{AMX = \rho \frac{\pi D^2}{4} C_{mt}} \f$
- \f$ \mathrm{AMY = \rho \frac{\pi D^2}{4} C_{mn}} \f$

where:

- \f$ \mathrm{\rho} \f$: water density
- \f$ \mathrm{g} \f$: acceleration of gravity
- \f$ \mathrm{S_W} \f$: cross sectional wetted surface \f$ \mathrm{(= \pi D)} \f$
- \f$ \mathrm{D} \f$: hydrodynamic diameter of the pipe
- \f$ \mathrm{C_{dt} } \f$: nondimensional quadratic tangential drag coefficient
- \f$ \mathrm{C_{dn} } \f$: nondimensional quadratic normal drag coefficient
- \f$ \mathrm{C^L_{dt} } \f$: nondimensional linear tangential drag coefficient
- \f$ \mathrm{C^L_{dn} } \f$: nondimensional linear normal drag coefficient
- \f$ \mathrm{C_{mt} } \f$: nondimensional tangential added mass coefficient
- \f$ \mathrm{C_{mn} } \f$: normal added mass coefficient
    - (\f$ \mathrm{C_{mn} } \f$ is normally equal to 1.0 for a circular cross section)
...
- `AMY: real, default: 0.0`: Added mass per unit length in normal direction \f$ \mathrm{ [M/L] } \f$
...
- `CDXAERO: real`: Quadratic drag coefficient in tangential direction
    - `ICODE=1: CDXAERO=CDXa`: dimensional drag force coefficient \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
    - `ICODE=2: CDXAERO=Cdta`: non-dimensional drag force coefficient
- `CDYAERO: real`: Quadratic drag coefficient in normal direction
    - `ICODE=1: CDYAERO=CDYa`: dimensional drag force coefficient \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
...
- `ICODE: integer, default: 1`: Code for input of aerodynamic force coefficients
    - `ICODE=1`: Dimensional coefficients
    - `ICODE=2`: Nondimensional coefficients
- `D: real, default:`\f$ \sqrt{\mathrm{\frac{4}{\pi}(AE) }} \f$: Aerodynamic diameter of the pipe \f$ \mathrm{ [L] } \f$.
    - Default value is calculated from external cross-sectional area given as input
    in data section [Mass and volume](@ref inpmod_c_crs1_mass_and_volume)
    - Dummy for `ICODE=1`
...
The tangential force which is a friction force per unit length acting in local x-axis,
 \f$ \mathrm{F_t} \f$ is computed by:

\f$ \mathrm{F_t = CDXa \times VRELX \times |VRELX| } \f$

The drag force per unit length acting normal to the local x-axis, \f$ \mathrm{F_n} \f$, 
...
\f$ \mathrm{F_n = CDYa \left(VRELY^2 + VRELZ^2 \right) } \f$

where:
- \f$ \mathrm{CDXa, CDYa} \f$: are the dimensional quadratic drag force coefficients in local x- and y-directions (i.e. tangential and normal directions)
- \f$ \mathrm{VRELX, VRELY, VRELZ} \f$: are relative wind velocities in local x,y and z-directions
...
- \f$ \mathrm{CDXa = \frac{1}{2} \rho_a S_W C_{dta}} \f$
- \f$ \mathrm{CDYa = \frac{1}{2} \rho_a D C_{dna}} \f$

where:

- \f$ \mathrm{\rho_a} \f$: air density
- \f$ \mathrm{S_W} \f$: cross sectional perimeter \f$ \mathrm{(= \pi D)} \f$
- \f$ \mathrm{D} \f$: aerodynamic diameter of the pipe
- \f$ \mathrm{C_{dta} } \f$: nondimensional quadratic tangential drag coefficient
- \f$ \mathrm{C_{dna} } \f$: nondimensional quadratic normal drag coefficient
...
- `TB: real`: Tension capacity \f$ \mathrm{[F]} \f$
- `YCURMX: real`: Maximum curvature \f$ \mathrm{[1/L]} \f$
...
- `AMS: real`: Mass per unit length \f$ \mathrm{[M/L]} \f$
- `AE: real`: External cross-sectional area \f$ \mathrm{ \left[L^2 \right]} \f$
- `AI: real`: Internal cross-sectional area \f$ \mathrm{ \left[L^2 \right]} \f$
- `RGYR: real`: Radius of gyration about local x-axis \f$ \mathrm{[L]} \f$
...
- `EJY: real > 0`: Bending stiffness around local y-axis \f$ \mathrm{ \left[ FL^2 \right]} \f$
- `EJZ: real > 0`: Bending stiffness around z-axis \f$ \mathrm{ \left[ FL^2 \right]} \f$
- `GAsZ: real`: Shear stiffness in Z-direction \f$ \mathrm{[F]} \f$
- `GAsY: real`: Shear stiffness in Y-direction \f$ \mathrm{[F]} \f$
...
- `EJY(1): real`: Bending stiffness around local y-axis \f$ \mathrm{ \left[ FL^2 \right]} \f$
- `EJZ(1): real`: Bending stiffness around local z-axis \f$ \mathrm{ \left[ FL^2 \right]} \f$
- `PRESS(1): real`: Pressure at which the above values apply \f$ \mathrm{ \left[ F/L^2 \right]} \f$
...
- `BMOMY(1): real`: Bending moment around local y-axis \f$ \mathrm{[FL]} \f$
- .
- .
- .
- `BMOMY(N): real`
...
- `BMOMZ(1): real`: Bending moment around local z-axis \f$ \mathrm{[FL]} \f$
- .
- .
- .
- `BMOMZ(N): real`
...
- `CURV(1): real`: Curvature values for which bending moments are specified \f$ \mathrm{[1/L]} \f$
- .
- .
- .
- `CURV(N): real`: To be specified in increasing order
...
- `PRESS(1): real`: Pressure levels for which bending moments are specified \f$ \mathrm{ \left[ F/L^2 \right]} \f$
- .
- .
- .
- `PRESS(N): real`:
...
- `BMOMY(I,J): real`: Bending moment about local y-axis at curvature I and pressure J \f$ \mathrm{[FL]} \f$.
- .
- .
- .
- `BMOMY(N1,N2):real`:
...
- `BMOMZ(I,J): real`: Bending moment about local Z-axis at curvature I and pressure J \f$ \mathrm{[FL]} \f$.
- .
- .
- .
- `BMOMZ(N1,N2):real`:
...
- `GT-: real > 0`: Torsion stiffness (negative twist) \f$ \mathrm{ \left[ FL^2/Radian \right] } \f$
...
- `CDX: real`: Drag force coefficient for local x-direction \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDY: real`: Drag force coefficient for local y-direction \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDZ: real`: Drag force coefficient for local z-direction \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDTMOM: real`: Drag force coefficient for local x-rotation. Not used in present version.
- `AMX: real`: Added mass per length in x-direction \f$ \mathrm{[M/L]} \f$
- `AMY: real`: Added mass per length in y-direction \f$ \mathrm{[M/L]} \f$
- `AMZ: real`: Added mass per length in z-direction \f$ \mathrm{[M/L]} \f$
- `AMTOR: real`: Added mass for local x-rotation \f$ \mathrm{\left[ ML^2/L \right]} \f$ Not used in present version.
- `CDLX: real, default: 0`: Linear drag force coefficients in local x-direction \f$ \mathrm{\left[F/\left((L/T) \times L\right)\right]} \f$
- `CDLY: real, default: 0`: Linear drag force coefficients in local y-direction \f$ \mathrm{\left[F/\left((L/T) \times L\right)\right]} \f$
- `CDLZ: real, default: 0`: Linear drag force coefficients in local z-direction \f$ \mathrm{\left[F/\left((L/T) \times L\right)\right]} \f$
...
- \f$ \mathrm{F_x = CDX \times VRELX \times VRELX + CDLX \times VRELX } \f$
- \f$ \mathrm{F_y = CDY \times \sqrt{VRELY^2 + VRELZ^2} \times VRELY  + CDLY \times VRELY  } \f$
- \f$ \mathrm{F_z = CDZ \times \sqrt{VRELY^2 + VRELZ^2} \times VRELZ  + CDLZ \times VRELZ  } \f$

where:
- \f$ \mathrm{CDX, CDY, CDZ} \f$: are the input quadratic drag force coefficients in local x, y and z-directions
- \f$ \mathrm{CDLX, CDLY, CDLZ} \f$: are the input linear drag force coefficients in local x, y and z-directions
- \f$ \mathrm{VRELX, VRELY, VRELZ} \f$: are relative water velocities in local x, y and z-directions

The input quadratic drag force coefficients \f$ \mathrm{CDX}\f$, \f$ \mathrm{CDY}\f$ and \f$ \mathrm{CDZ}\f$ will normally be calculated as:
- \f$ \mathrm{CDX = \frac{1}{2} \rho S_{2D} C_{dx}} \f$
- \f$ \mathrm{CDY = \frac{1}{2} \rho B_y C_{dy}} \f$
- \f$ \mathrm{CDZ = \frac{1}{2} \rho B_z C_{dz}} \f$

where:
- \f$ \mathrm{\rho} \f$: water density
- \f$ \mathrm{S_{2D}} \f$: cross sectional wetted surface
- \f$ \mathrm{B_y, B_z} \f$: projected area per. unit length for flow in local y and z-direction, respectively
- \f$ \mathrm{C_{dx},C_{dy},C_{dz}} \f$: nondimensional drag coefficients in local x, y and z-directions, respectively

The input added mass per. unit length \f$ \mathrm{AMX}\f$, \f$ \mathrm{AMY}\f$ and \f$ \mathrm{AMZ}\f$ can be calculated as:
- \f$ \mathrm{AMX = \rho A C_{mx} } \f$
- \f$ \mathrm{AMY = \rho A C_{my}} \f$
- \f$ \mathrm{AMZ = \rho A C_{mz} } \f$

where:
- \f$ \mathrm{\rho} \f$: water density
- \f$ \mathrm{A} \f$: cross sectional area
- \f$ \mathrm{C_{mx},C_{my},C_{mz}} \f$: nondimensional added mass coefficients in local x, y and z-directions, respectively
...
- `CQZ: real`: Quadratic drag coefficient in local z-direction 
    - `ICODE=1: CQZ=CDZ`: dimensional drag force coefficient \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
    - `ICODE=2: CQZ=Cdn`: nondimensional drag force coefficient
- `ICODE: integer, default: 1`: `ICODE` Code for input of hydrodynamic force coefficients
    - `ICODE=1`: Dimensional coefficients
    - `ICODE=2`: Nondimensional coefficients
- `D: real, default:`\f$ \sqrt{\mathrm{\frac{4}{\pi}(AE) }} \f$: Hydrodynamic diameter \f$ \mathrm{ [L] } \f$.
    - Default value is calculated from external cross-sectional area given as input 
...
- `CDXAERO: real`: Dimensional quadratic drag coefficient for local x-direction \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDYAERO: real`: Dimensional quadratic drag coefficient for local y-direction \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDZAERO: real`: Dimensional quadratic drag coefficient for local z-direction \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$

The drag forces per unit length acting in the local coordinate system are computed as:
- \f$ \mathrm{F_x = CDXAERO \times VRELX \times |VRELX| } \f$
- \f$ \mathrm{F_y = CDYAERO \times VRELY \times \sqrt{VRELY^2 + VRELZ^2} } \f$
- \f$ \mathrm{F_z = CDZAERO \times VRELZ \times \sqrt{VRELY^2 + VRELZ^2} } \f$

where:
- \f$ \mathrm{CDXAERO, CDYAERO, CDZAERO} \f$: are the input quadratic drag force coefficients in local x, y and z-directions
- \f$ \mathrm{VRELX, VRELY, VRELZ} \f$: are relative wind velocities in local x, y and z-directions

The input quadratic drag force coefficients \f$ \mathrm{CDX}\f$, \f$ \mathrm{CDY}\f$ and \f$ \mathrm{CDZ}\f$ will normally be calculated as:
- \f$ \mathrm{CDXAERO = \frac{1}{2} \rho_{air} S_{2D} C_{dx}} \f$
- \f$ \mathrm{CDYAERO = \frac{1}{2} \rho_{air} B_y C_{dy}} \f$
- \f$ \mathrm{CDZAERO = \frac{1}{2} \rho_{air} B_z C_{dz}} \f$

where:
- \f$ \mathrm{\rho_{air}} \f$: air density
- \f$ \mathrm{S_{2D}} \f$: cross sectional surface area
- \f$ \mathrm{B_y, B_z} \f$: projected area per. unit length for flow in local y and z-direction, respectively
- \f$ \mathrm{C_{dx},C_{dy},C_{dz}} \f$: nondimensional drag coefficients in local x, y and z-directions, respectively
...
- `CHORDL: real`: Chord length of foil section. \f$ \mathrm{[L]}\f$
    - It is used to scale the air foil load coefficients.
- `YFC: real, default: 0`: Y-coordinate of foil origin \f$ \mathrm{[L]}\f$
- `ZFC: real, default: 0`: Z-coordinate of foil origin \f$ \mathrm{[L]}\f$
- `ROTFAX: real, default: 0`: Inclination of foil system \f$ \mathrm{[deg]}\f$

The blade coordinate system and origin coincides with the elastic (local) \f$ \mathrm{(X_L,Y_L,Z_L)}\f$  coordinate system. 
The aerodynamic coordinate system \f$ \mathrm{(X_{AF},Y_{AF})}\f$ is located at (YFC,ZFC) referred to the local coordinate system, 
and is rotated about the blade x axis by the angle ROTFAX, as indicated in the figure below. The  \f$ \mathrm{X_L}\f$ axis is 
pointing into the paper plane, while the  \f$ \mathrm{Z_{AF}}\f$ is pointing out of plane. Note that the air foil coefficients 
...
elements that are part of a wind turbine blade, the local \f$ \mathrm{X_L}\f$-axis is pointing towards the blade tip.


Note that suppliers of wind turbine blades normally give the foil twist relative to the the areodynamic 
coordinate system, i.e. as twist around the \f$ \mathrm{Z_{AF}}\f$ -axis.
...
- `TB: real`: Tension capacity \f$ \mathrm{[F]}\f$
- `YCYRMX: real`: Maximum curvature around local y-axis \f$ \mathrm{[1/L]}\f$
- `ZCURMX: real`: Maximum curvature around local z-axis \f$ \mathrm{[1/L]}\f$
...
- `TEMP : real`: Temperature at which the specification applies 
    - Dummy in present version
- `ALPHA: real`: Thermal expansion coefficient \f$ \mathrm{\left[ Temp^{-1} \right] } \f$
	- Dummy in present version
...
- `YECC_MASS: real`: Mass center coordinate \f$ Y_m \f$ in beam element system \f$ \mathrm{[L]} \f$
- `ZECC_MASS: real`: Mass center coordinate \f$ Z_m \f$ in beam element system \f$ \mathrm{ \left[L\right]} \f$
...
- `AMS : real`: Mass per unit length \f$ \mathrm{[M/L]} \f$
- `RGYR: real`: Radius of gyration about mass center \f$ (Y_m,Z_m) \f$ \f$ \mathrm{[L]} \f$
...
- `YECC_BUOY: real`: Buoyancy center Y-coordinate in beam element system \f$ \mathrm{[L]} \f$
	- Dummy in present version. Bouyancy center set equal to mass center.
- `ZECC_BUOY: real`: Buoyancy center Z-coordinate in beam element system \f$ \mathrm{ \left[L\right]} \f$
	- Dummy in present version. Bouyancy center set equal to mass center.
...
from the requirement \f$ \int_A V \, W  \,\,\mathrm{d}A = 0 \f$, where \f$ V \f$ and \f$ W \f$ denote the principal coordinates 
and \f$ A \f$ is the cross-section area. The orientation of the principal axes is defined in terms of a positive X-rotation 
\f$ \theta \f$ relative to the beam element YZ-coordinate system as shown in the figure [General cross-section](@ref gen_nonsym_crossec)
~~~
YECC_AREACENT ZECC_AREACENT THETA
~~~

- `YECC_AREACENT: real`: Area center coordinate \f$ Y_a \f$ in beam element system \f$ \mathrm{[L]} \f$
- `ZECC_AREACENT: real`: Area center coordinate \f$ Z_a \f$ in beam element system \f$ \mathrm{ \left[L\right]} \f$
- `THETA: real`: Orientation \f$ \theta \f$ of principal axes V and W [deg.]. See figure [General cross-section](@ref gen_nonsym_crossec).
...
- `YECC_SHEARCENT: real`: Shear center coordinate \f$ Y_s \f$ in beam element system \f$ \mathrm{[L]} \f$
- `ZECC_SHEARCENT: real`: Shear center coordinate \f$ Z_s \f$ in beam element system \f$ \mathrm{ \left[L\right]} \f$
...
- `EJV: real > 0`: Bending stiffness about principal V-axis \f$ \mathrm{ \left[ FL^2 \right]} \f$
- `EJW: real > 0`: Bending stiffness about principal W-axis \f$ \mathrm{ \left[ FL^2 \right]} \f$
...
- `GAsW: real`: Shear stiffness in principal W-direction \f$ \mathrm{[F]} \f$
- `GAsV: real`: Shear stiffness in principal V-direction \f$ \mathrm{[F]} \f$
...
- `AMY: real`: Added mass per length in y-direction \f$ \mathrm{[M/L]} \f$
- `AMZ: real`: Added mass per length in z-direction \f$ \mathrm{[M/L]} \f$
- `AMTOR: real`: Added mass for local x-rotation \f$ \mathrm{\left[ ML^2/L \right]} \f$
- `CDLX: real, default: 0`: Linear drag force coefficients in local x-direction \f$ \mathrm{\left[F/\left((L/T) \times L\right)\right]} \f$
- `CDLY: real, default: 0`: Linear drag force coefficients in local y-direction \f$ \mathrm{\left[F/\left((L/T) \times L\right)\right]} \f$
- `CDLZ: real, default: 0`: Linear drag force coefficients in local z-direction \f$ \mathrm{\left[F/\left((L/T) \times L\right)\right]} \f$
- `SCFKN: real, default: 1`: Scaling factor for the Froude-Krylov term in Morison’s equation in normal direction
...
The drag forces per unit length acting in the local coordinate system are computed as:
- \f$ \mathrm{F_x = CDX \times VRELX \times VRELX + CDLX \times VRELX } \f$
- \f$ \mathrm{F_y = CDY \times VRELY \times VRELY + CDLY \times VRELY } \f$
- \f$ \mathrm{F_z = CDZ \times VRELZ \times VRELZ + CDLZ \times VRELZ } \f$
...
- `AM: real`: Mass \f$ \mathrm{[M]}\f$
- `AE: real`: Displacement volume \f$ \mathrm{ \left[L^3 \right]}\f$
...
- `CDX: real`: Drag force coefficient in X-direction \f$ \mathrm{\left[F/\left(L/T)^2 \right)\right]} \f$
- `CDY: real`: Drag force coefficient in Y-direction \f$ \mathrm{\left[F/\left(L/T)^2 \right)\right]} \f$
- `CDZ: real`: Drag force coefficient in Z-direction \f$ \mathrm{\left[F/\left(L/T)^2 \right)\right]} \f$
- `AMX: real`: Added mass in X-direction \f$ \mathrm{[M]}\f$
- `AMY: real`: Added mass in Y-direction \f$ \mathrm{[M]}\f$
- `AMZ: real`: Added mass in Z-direction \f$ \mathrm{[M]}\f$

The drag forces acting in the global/local coordinate system are computed as:
- \f$ \mathrm{F_x = CDX \times VRELX \times VRELX } \f$
- \f$ \mathrm{F_y = CDY \times VRELY \times VRELY } \f$
- \f$ \mathrm{F_z = CDZ \times VRELZ \times VRELZ } \f$

where:
- \f$ \mathrm{CDX, CDY, CDZ} \f$: are the input drag force coefficients in global/local x, y and z-directions, respectively
- \f$ \mathrm{VRELX, VRELY, VRELZ} \f$: are relative water velocities in global/local x, y and z-directions respectively

The input quadratic drag force coefficients \f$ \mathrm{CDX}\f$, \f$ \mathrm{CDY}\f$ and \f$ \mathrm{CDZ}\f$ will normally be calculated as:
- \f$ \mathrm{CDX = \frac{1}{2} \rho B_x C_{dx}} \f$
- \f$ \mathrm{CDY = \frac{1}{2} \rho B_y C_{dy}} \f$
- \f$ \mathrm{CDZ = \frac{1}{2} \rho B_z C_{dz}} \f$

where:
- \f$ \mathrm{\rho} \f$: water density
- \f$ \mathrm{B_x,B_y,B_z} \f$: projected area for flow in global/local y and z-direction
- \f$ \mathrm{C_{dx},C_{dy},C_{dz}} \f$: nondimensional drag coefficients in global/local x, y and z-directions
...
- `AM: real`: Mass \f$ \mathrm{[M]}\f$
- `AE: real`: Displacement volume \f$ \mathrm{[L^3]}\f$
...
where:
- \f$ \mathrm{\rho} \f$: water density
- \f$ \mathrm{B_x,B_y,B_z} \f$: projected area per. unit lengt for flow in global/local y and z-directions, respectively
- \f$ \mathrm{C_{dx},C_{dy},C_{dz}} \f$: nondimensional drag coefficients in global/local x, y and z-directions, respectively
...
- `AM: real, default: 0`: Mass \f$ \mathrm{[M]}\f$
- `AE: real, default: 0`: Displacement volume \f$ \mathrm{ \left[ L^3 \right]}\f$
- `RGX: real, default: 0`: Radius of gyration around local x-axis \f$ \mathrm{[L]}\f$
- `RGY: real, default: 0`: Radius of gyration around local y-axis \f$ \mathrm{[L]}\f$
- `RGZ: real, default: 0`: Radius of gyration around local z-axis \f$ \mathrm{[L]}\f$
- `CRX: real, default: 0`: Damping coeff. Rotational velocity around local x-axis \f$ \mathrm{[FLT/deg]}\f$
- `CRY: real, default: 0`: Damping coeff. Rotational velocity around local y-axis \f$ \mathrm{[FLT/deg]}\f$
- `CRZ: real, default: 0`: Damping coeff. Rotational velocity around local z-axis \f$ \mathrm{[FLT/deg]}\f$
...
- `CDX: real, default: 0`: Drag coeff. in local x-direction \f$ \mathrm{\left[F/\left(L/T)^2 \right)\right]} \f$
- `CDY: real, default: 0`: Drag coeff. in local y-direction \f$ \mathrm{\left[F/\left(L/T)^2 \right)\right]} \f$
- `CDZ: real, default: 0`: Drag coeff. in local z-direction \f$ \mathrm{\left[F/\left(L/T)^2 \right)\right]} \f$
- `AMX: real, default: 0`: Added mass in local x-direction \f$ \mathrm{[M]}\f$
- `AMY: real, default: 0`: Added mass in local y-direction \f$ \mathrm{[M]}\f$
- `AMZ: real, default: 0`: Added mass in local z-direction \f$ \mathrm{[M]}\f$
- `AMXROT: real, default: 0`: Added mass rotation around local x-direction \f$ \mathrm{[FL \times T^2]}\f$
- `AMYROT: real, default: 0`: Added mass rotation around local y-direction \f$ \mathrm{[FL \times T^2]}\f$
- `AMZROT: real, default: 0`: Added mass rotation around local z-direction \f$ \mathrm{[FL \times T^2]}\f$

The tangential drag force, the force acting in local x-axis, is computed by:

\f$ \mathrm{FX = CDX \times VRELX \times |VRELX|} \f$
...
- \f$ \mathrm{FY = CDY \times \sqrt{VRELY^2 + VRELZ^2} \times VRELY} \f$
- \f$ \mathrm{FZ = CDY \times \sqrt{VRELY^2 + VRELZ^2} \times VRELZ} \f$
...
- `STIFF: real`: stiffness with respect to rotation \f$ \mathrm{[FL/deg]}\f$
...
- `MOMENT: real`: Moment corresponding to rotational angle \f$ \mathrm{[FL]}\f$
- `ANGLE: real`: Rotational angle \f$ \mathrm{[deg]}\f$
...
- `RHOI: real`: Density \f$\left[ \mathrm{M/L^3} \right]\f$
- `VVELI: real`: Volume velocity \f$\left[ \mathrm{L^3/T} \right]\f$
- `PRESSI: real`: Pressure at fluid inlet end \f$\left[ \mathrm{F/L^2} \right]\f$
- `DPRESS: real`: Pressure drop \f$\left[ \mathrm{F/L^3} \right]\f$
...
- `AMS: real`: Mass per unit length \f$ \mathrm{[M/L]} \f$
- `AE: real`: Buoyancy volume/length \f$ \mathrm{ \left[L^2 \right]} \f$
- `RGYR: real`: Radius of gyration around local x-axis \f$ \mathrm{[L]} \f$
- `FRAC: real`: Fraction of the segment that is covered \f$ \mathrm{[1]} \f$
    - `0 <= FRAC <= 1`

The resulting properties of the segment with external wrapping are:

Mass / length:
- \f$ \mathrm{AMS_{res} = AMS_{cs} + AMS_{ext} \times FRAC } \f$

Resulting radius of gyration:
- \f$ \mathrm{RGYR_{res} = \left( AMS_{cs} \times RGYR_{cs}^2 + AMS_{ext} \times RGYR_{ext}^2 \times FRAC \right) / \left( AMS_{cs} + AMS_{ext} \times FRAC \right) } \f$

Resulting external area for buoyancy:
- \f$ \mathrm{AE_{res} = AE_{cs} + AE_{ext} \times FRAC } \f$
...
- `CDX: real`: Drag force coefficient in tangential direction \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDY: real`: Drag force coefficient in normal direction \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `AMX: real`: Added mass per length in tangential direction \f$ \mathrm{[M/L]} \f$
- `AMY: real`: Added mass per length in normal direction \f$ \mathrm{[M/L]} \f$
- `CDLX: real, default: 0`: Linear drag force coefficients in tangential direction \f$ \mathrm{\left[F/\left((L/T) \times L\right)\right]} \f$
- `CDLY: real, default: 0`: Linear drag force coefficients in normal direction \f$ \mathrm{\left[F/\left((L/T) \times L\right)\right]} \f$
...
- \f$ \mathrm{F_x = \left(CDX_R + FRAC \times CDX \right) VRELX \times VRELX + \left(CDLX_R + FRAC \times CDLX \right) VRELX } \f$

In the case of an axis symmetric cross section the drag force per unit length, \f$ \mathrm{F_n}\f$, acting normal to
the local x-axis is computed by assuming that the instantaneous drag force direction is parallel to
the instantaneous transverse relative velocity component
- \f$ \begin{array}{l} \mathrm{F_n = (CDY_R + FRAC \times CDY) \left(VRELY^2 + VRELZ^2 \right)} \\ + \mathrm{(CDLY_R + FRAC \times CDLY) \sqrt{VRELY^2 + VREZ^2} } \end{array} \f$
...
- \f$ \mathrm{F_y = (CDY_R + FRAC \times CDY) VRELY \times VRELY + (CDLY_R + FRAC \times CDLY) VRELY } \f$
- \f$ \mathrm{F_z = (CDZ_R + FRAC \times CDZ) VRELZ \times VRELZ + (CDLZ_R + FRAC \times CDLZ) VRELZ } \f$

Where:
- \f$ \mathrm {CDX_R, CDY_R, CDZ_R} \f$: are the input quadratic drag force coefficients of the
    riser in local x,y and z-directions
- \f$ \mathrm {CDLX_R, CDLY_R, CDLZ_R} \f$: are the input linear drag force coefficient of the riser
    in local x,y and z-directions
- \f$ \mathrm {CDX, CDY} \f$: are the input quadratic drag force coefficients of the
    external wrapping in local x and y-directions
- \f$ \mathrm {CDLX, CDLY} \f$: are the input linear drag force coefficients of the
    external wrapping in local x and y-directions
- \f$ \mathrm {VRELX, VRELY, VRELZ} \f$: are relative water velocities in local x, y and
    z-directions
...
- \f$ \mathrm{CDY = \frac{1}{2} \rho \left( DC_{dn} - D_R C_{dnR} \right) } \f$

The added mass per unit length in normal direction can be calculated as:
- \f$ \mathrm{AMY = \rho \frac{\pi}{4} \left( D^2C_{mn} - D_R^2 C_{mnR} \right) } \f$

where:
- \f$ \mathrm{\rho} \f$: water density
- \f$ \mathrm{D} \f$: outer diameter of the external wrapping
- \f$ \mathrm{D_R} \f$: outer diameter of the pipe
- \f$ \mathrm{C_{dn}} \f$: normal drag coefficient
- \f$ \mathrm{C_{dnR}} \f$: normal drag coefficient of the pipe
- \f$ \mathrm{C_{mn}} \f$: normal added mass coefficient of the external wrapping
- \f$ \mathrm{C_{mnR}} \f$: normal added mass coefficient of the pipe
...
values are taken proportional with the instantaneous cross section submergence. The actual submergence is based on an equivalent circular control area identical to the external area, \f$ \mathrm {AE} \f$.
...
- \f$ \mathrm {F_y = CDY \times (A_{sub}/AE) \times \sqrt{(VRELY^2 + VRELZ^2)} \times VRELY} \f$
- \f$ \mathrm {F_z = CDZ \times (A_{sub}/AE) \times \sqrt{(VRELY^2 + VRELZ^2)} \times VRELZ} \f$

where \f$ \mathrm {A_{sub}} \f$ is the instantaneous submerged area of the cross section.
...
are restricted to 30\f$ ^{\circ}\f$ as a practical upper limit. The stiffness
...
- `CDX: real`: Drag force coefficient per length, tangential \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDY: real`: Drag force coefficient per length, local y-axis \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDZ: real`: Drag force coefficient per length, local z-axis \f$ \mathrm{\left[F/\left((L/T)^2 \times L\right)\right]} \f$
- `CDTMOM: real`: Drag coefficient around local x-axis
    - Dummy in present version.
- `AMX: real`: Added mass per length, tangential \f$ \mathrm{[M/L]} \f$
...
according to:
- \f$ \mathrm{F_t = CDX \times (A_{sub}/A_{tot}) \times V_{x,rel} \times |V_{x,rel}| } \f$ 
...
component:
- \f$ \mathrm{F_y = CDY \times (A_{sub}/A_{tot}) \times \sqrt{V^2_{y,rel} \times V^2_{z,rel}} \times V_{y,rel} } \f$  
- \f$ \mathrm{F_z = CDZ \times (A_{sub}/A_{tot}) \times \sqrt{V^2_{y,rel} \times V^2_{z,rel}} \times V_{z,rel} } \f$  

where:
- \f$ \mathrm{A_{sub}} \f$: is instantaneous cross section submergence
- \f$ \mathrm{A_{tot}} \f$: is total external areal of the cross section
- \f$ \mathrm{V_{x,rel}} \f$: is relative water velocity in local x-direction
- \f$ \mathrm{V_{y,rel}} \f$: is relative water velocity in local y-direction
- \f$ \mathrm{V_{z,rel}} \f$: is relative water velocity in local z-direction
...
roll angles in the range (`0` - \f$ \pi\f$/`2`). The instantaneous submerged area is found by linear interpola-
...
The local coordinate system \f$ \mathrm{ \left( X_L, Y_L, Z_L \right) }\f$ of the elastic contact surface is indicated. 
The \f$ \mathrm{X_L}\f$-axis is pointing into the paper plane.

The contact point may contain several rollers.

The rollers are located in the \f$ \mathrm{Y_LZ_L}\f$-plane of the element to which the contact point is attached. Besides
...
in the figure below. The roller origin (starting point) is defined by the point \f$ \mathrm{ \left( Y_R, Z_R \right) }\f$ and the inclination angle
(`ROTX`) is defined by a clockwise rotation around the contact surface \f$ \mathrm{X_L}\f$-axis.
...
- `ROTX: real, default: 0`: Direction of roller axis. (Clockwise around the \f$ \mathrm{X_L}\f$-axis of the actual
surface plane) \f$ \mathrm{[deg]}\f$
- `YR: real, default: 0`: Y-coordinate of roller origin \f$ \mathrm{[L]}\f$
- `ZR: real, default: 0`: Z-coordinate of roller origin \f$ \mathrm{[L]}\f$
- `RLENG: real, default: 0`: Length of roller \f$ \mathrm{[L]}\f$
...
- `STIFFR: real`: Spring compression stiffness \f$ \mathrm{[F/L]}\f$
- `RADROL: real`: Radius of roller \f$ \mathrm{[L]}\f$
...
- `FS(1): real < 0`: Pressure force corresponding to compression `ZS(1)` \f$ \mathrm{[F]}\f$
- `ZS(1): real`: Spring compression \f$ \mathrm{[L]}\f$
- .
- .
- .
...
- `T0: real`: Applied load during static analysis \f$ \mathrm{[F]}\f$
- `TMAX: real`: Maximum load transmitted from the tensioner \f$ \mathrm{[F]}\f$
- `TMIN: real`: Minimum load transmitted from the tensioner \f$ \mathrm{[F]}\f$
- `DELTA: real`: Pipeline displacement through the tensioner for a load variation of: `TMAX-TMIN`  \f$ \mathrm{[L]}\f$
- `SIGNX: real, default: 1`: Direction of applied load referring to local x-axis of the element going through the tensioner \f$ \mathrm{[]}\f$
...
- `CONT: real`: Contact radius  \f$ \mathrm{[L]}\f$
- `CHDIR: character`: Contact direction: `INWARDS` or `OUTWARDS` 
- `IKS: integer`: Stiffness code for radial contact force 
    - `= 1` Constant contact compression stiffness
    - `= N` Table with N pairs of contact force - displacement to be specified
- `RELDAM: real`: = Desired relative damping level at estimated eigen
   period in the pipe, pipe and contact spring system (see below) \f$
   \mathrm{[1]}\f$. Damping is only applied in the radial
    direction. Not used in static analysis. 
- `DAMP: real`: Dash pot damping coefficient \f$
  \mathrm{[FT/L]}\f$. Damping is only applied in the radial
  direction. Not used in static analysis.
- `STIFFR: real`: Spring stiffness associated with static friction
  coefficient `FRICST`, \f$ \mathrm{[F/L]}\f$. The spring stiffness is
...
\f$ \mathrm{a_2 = RELDAM \times 2 \times \sqrt \frac{(AMS \times L)_M + (AMS \times L)_S }{STIFF}}\f$

where \f$ \mathrm{(AMS \times L)_M}\f$ and \f$ \mathrm{(AMS \times L)_S}\f$
 are total structural element mass of the master pipe and the slave pipe 
respectively and \f$ \mathrm{STIFF}\f$ is contact spring sitffness.
...
- `STIFF: real`: Spring compression stiffness \f$ \mathrm{[F/L]}\f$
...
- `FS(1): real < 0`: Pressure force corresponding to compression `ZS(1)` \f$ \mathrm{[F]}\f$
- `ZS(1)`: Spring compression \f$ \mathrm{[L]}\f$
...
seafloor, \f$ k_V \f$ , for seafloor contact. \f$ k_V \f$ = `STFBOT`
\f$\times L\f$ where \f$ L \f$ is the element length.
...
stiffness is calculated as \f$ k_h = \mathrm{Stalks} \times L_h \f$,
where \f$ L_h \f$ is the length of the element's horizontal
projection.  The seafloor friction forces are calculated as \f$ F =
\mathrm {FRIxxx} \times F_{vert} \f$ and are directed against the
axial or lateral displacements, where \f$ F_{vert}\f$ is the vertical
...
- `W: real > 0`: Soil submerged weight \f$ \mathrm{ \left[F/L^3 \right]}\f$
- `A1: real > 0`: Soil shear strength at seabed \f$ \mathrm{ \left[F/L^2 \right]}\f$
- `A2: real`: Soil shear strength vertical gradient \f$ \mathrm{ \left[F/L^3 \right]}\f$
- `V: real > 0`: Soil Poisson ratio \f$ \mathrm{ \left[1 \right]}\f$
- `G: real`: Soil G-modulus/compressive strength \f$ \mathrm{ \left[F/L^2 \right]}\f$
...
- `F: real, default: 0.88`: Relationship between dynamic stiffness and G-modulus \f$ \mathrm{ \left[1 \right]}\f$
- `ALPHA: real, default: 1.0`: Control parameter for suction release displacement \f$ \mathrm{ \left[1 \right]}\f$
- `BETA: real, default: 1.0`: Scaling factor for peak soil suction relative to peak soil compression \f$ \mathrm{ \left[1 \right]}\f$
- `KBC: real, default: 0.05`: Mobilization displacement for soil bearing capacity as fraction of pipe soil contact width \f$ \mathrm{ \left[1 \right]}\f$
- `KT: real, default: 0.08`: Mobilization displacement for max soil suction as fraction of pipe soil contact width \f$ \mathrm{ \left[1 \right]}\f$
...
- `UFA1: real, default: 1`: Factor for converting default values from \f$ \mathrm{ \left[kN/m^2 \right]}\f$ to actual unit for \f$ \mathrm{ \left[F/L^2 \right]}\f$
- `UFA2: real, default: 1`: Factor for converting default values from \f$ \mathrm{ \left[kN/m^3 \right]}\f$ to actual unit for \f$ \mathrm{ \left[F/L^3 \right]}\f$
...
- `RKU: real, default: 145`: Longitudinal stiffness per unit pipe length  \f$ \mathrm{ \left[F/L^2 \right]}\f$
- `RKV: real, default: 145`: Transverse stiffness per unit pipe length  \f$ \mathrm{ \left[F/L^2 \right]}\f$
- `RKW: real, default: 200`: Vertical stiffness per unit pipe length  \f$ \mathrm{ \left[F/L^2 \right]}\f$
- `RMUU: real, default: 0.7`: Axial friction coefficient 
- `RMUV: real, default: 0.7`: Transverse friction coefficient 

Indicated default values for parameters with dimension \f$ \mathrm{ \left[F/L^2 \right]}\f$ or \f$ \mathrm{ \left[F/L^3 \right]}\f$ are in \f$ \mathrm{ \left[kN/m^2 \right]}\f$ or
 \f$ \mathrm{ \left[kN/m^3 \right]}\f$ and will be scaled by `UFA1` or `UFA2` before being applied. See echo of input if in doubt.
...
- `ISTYP: character(4)`: `= SAND` 
- `PHI: real, default: 30`: Angle of friction  \f$ \mathrm{[deg]}\f$
- `WSOI: real, default: 9.1`: Submerged unit weight of soil  \f$ \mathrm{ \left[F/L^3 \right]}\f$
...
- `SU: real, default: 5`: Undrained shear strength \f$ \mathrm{ \left[F/L^3 \right]}\f$
- `WSOI: real, default: 4.4`: Submerged unit weight of soil  \f$ \mathrm{ \left[F/L^3 \right]}\f$
- `SUG: real, default: 0`: Shear strength gradient \f$ \mathrm{ \left[F/L^3 \right]}\f$
- `POI: real, default: 0.45`: Poisson's ratio \f$ \mathrm{[1]}\f$
- `ES: real, default: 2`: Void ratio \f$ \mathrm{[1]}\f$
- `PIX: real, default: 60`: Plasticity index \f$  [\mathrm{\%}]  \f$
...
- `RFVMX: real, default: 2.5`: Max transverse capacity ratio \f$ \mathrm{[1]}\f$
...
- `SU: real, default: 5`: Undrained shear strength \f$ \mathrm{ \left[F/L^3 \right]}\f$
- `WSOI: real, default: 4.4`: Submerged unit weight of soil  \f$ \mathrm{ \left[F/L^3 \right]}\f$
- `SU0: real, default: 0`: Shear strength gradient \f$ \mathrm{ \left[F/L^3 \right]}\f$
- `POI: real, default: 0.45`: Poisson's ratio \f$ \mathrm{[1]}\f$
- `ES: real, default: 2`: Void ratio \f$ \mathrm{[1]}\f$
- `PIX: real, default: 60`: Plasticity index \f$  [\mathrm{\%}]  \f$
- `OCR: real, default: 1`: Over-consolidation ratio \f$ \mathrm{[1]}\f$
- `RMUU: real, default: 0.2`: Axial friction coefficient 
- `RMUV: real, default: 0.2`: Transverse friction coefficient (not used)
- `RFVMX: real, default: 2.5`: Max transverse capacity ratio \f$ \mathrm{[1]}\f$
...
- `RKU: real, default: 145`: Fixed stiffness in axial direction (`ISSMO >= 0`) \f$ \mathrm{ \left[ F/L^2 \right]}\f$
    - Used for soil types `COUL`, `SAND` and `CLAY` and `ISSMO <= 0`
- `RKVI: real, default: 145`: Fixed stiffness in transverse direction (`ISSMO >= 0`) \f$ \mathrm{ \left[ F/L^2 \right]}\f$
    - Used for soil types `COUL`, `SAND` and `CLAY` and `ISSMO <= 0`
- `RKWI: real, default: 200`: Fixed stiffness in vertical direction (`ISSMO >= 0`) \f$ \mathrm{ \left[ F/L^2 \right]}\f$
    - Used for soil types `COUL`, `SAND` and `CLAY` and `ISSMO = 0`
- `RKWN: real, default: 10`: Minimum vertical stiffness \f$ \mathrm{ \left[ F/L^2 \right]}\f$
    - Used for soil types `COUL`, `SAND` and `CLAY` and `ISSMO` \f$ \neq \f$ `0`
- `RKWX: real, default: 15000`: Maximum vertical stiffness \f$ \mathrm{ \left[ F/L^2 \right]}\f$
    - Used for soil types `COUL`, `SAND` and `CLAY` and `ISSMO` \f$ \neq \f$ `0`
...
- `ISU: integer, default: 0`: Option for including soil suction
    ...
    - `= ± 2`: Suction force is related to then maximum contact force since last lift off (or laydown).
        - Reference length is the distance from the seafloor to the origin for vertical contact, but not more than one diameter.
    - `= ±`3 : Suction force is related to \f$ \mathrm{ S_u \times D} \f$
        - `= -3`: Reference length is footprint.
        - `= +3`: Reference length is diameter.
- `RF0: real, default: 0.25`: Suction force factor \f$ \mathrm{[1]}\f$
- `RC1: real, default: 0.7`: Total elongation \f$ \mathrm{[1]}\f$
- `RC2: real, default: 0.12`: Elastic fraction \f$ \mathrm{[1]}\f$
- `RC3: real, default: 0.7`: Degradation fraction \f$ \mathrm{[1]}\f$
- `RC4: real, default: 0.75`: Chusion force factor \f$ \mathrm{[1]}\f$

This data group is dummy for `ISTYP = COUL` or `SAND`.

  
- The vertical extent of suction over the origin for vertical contact is to `RC1` \f$ \times \f$ reference length.
- The `RC2` fraction of `RC1` \f$ \times \f$ reference length is the elastic fraction.
- The `RC3` fraction of `RC1` \f$ \times \f$ reference length is the degradation fraction.

The maximum suction force is `RC0` \f$ \times \f$  reference force. The maximum chusion force is `RC4` \f$ \times \f$ `RC0` \f$ \times \f$  reference force.
...
- `ISU: integer, default: 1`: Suction option
    - `= 0`: off
    - `> 0`: on 
- `CC: real, default: 0.63E7`: Consolidation coefficient \f$ \mathrm{ \left[T/L^2 \right]}\f$
- `TCO: real, default: 0.432E5`: Initial consolidation time \f$ \mathrm{[T]}\f$
- `PF0: real, default: -1`: Initial contact force expressed in penetration \f$ \mathrm{[1]}\f$
    - `> 0`: penetration given in diameters
    - `< 0`: penetration to trench depth 
- `PFN: real, default: 0`: Minimum contact force to be used expressed in penetration \f$ \mathrm{[1]}\f$
...
- `FLAG: character`: = `SDAM` 
- `RDA: real, default: See below`: Effective soil damping ratio \f$ \mathrm{[1]}\f$
- `TREF: real, default: 10`: Reference period for damping ratio \f$ \mathrm{[s]}\f$
- `DHFAC: real, default: 1`: Ratio of longitudinal and transverse damping to vertical damping

Stiffness proportional damping is applied in the vertical direction with the coefficient \f$ \mathrm{a2} \f$ found as 
\f$ \mathrm{a2 = RDA \times TREF / \pi} \f$. 
The \f$ \mathrm{a2} \f$ coefficients in the longitudinal and transverse directions are scaled by `DHFAC`.
...
- `TRSHI: real, default: 0`: Trench shift parameter \f$ \mathrm{[L]}\f$
- `TRLSC: real, default: 1`: Trench length scale factor \f$ \mathrm{[1]}\f$
- `TRDSC: real`: Trench depth scale factor \f$ \mathrm{[1]}\f$
...
- `TW: real`: Excess width \f$ \mathrm{[L]}\f$
...
|:--------:|:--------------------:|:-------------------------------------:|:----------------:|:------------------:|:--------------------:|
|          |\f$ \mathrm{\phi s}\f$| \f$ \mathrm{W_{soil}} \f$             | \f$\mathrm{n}\f$ | \f$\mathrm{e_s}\f$ | \f$\mathrm{\xi d}\f$ |
|          |\f$ \mathrm{[Deg]}\f$ | \f$ \mathrm{ \left[kN/m^3 \right]} \f$| -                | -                  | \f$\mathrm{[\%]}\f$  |
...
|:--------:|:-----------------------------------:|:-------------------------------------:|:----------------:|:------------------:|:--------------------:|:------------------:|
|          |\f$ \mathrm{S_u}\f$                  | \f$ \mathrm{W_{soil}} \f$             | \f$\mathrm{n}\f$ | \f$\mathrm{e_s}\f$ | \f$\mathrm{\xi d}\f$ | \f$\mathrm{i_p}\f$ |
|          |\f$ \mathrm{ \left[kN/m^3 \right]}\f$| \f$ \mathrm{ \left[kN/m^3 \right]} \f$| -                | -                  | \f$\mathrm{[\%]}\f$  | \f$\mathrm{[\%]}\f$|
...
- `LDC: real`: Drag chain length \f$ \mathrm{[L]} \f$
- `WDC: real`: Drag chain weight \f$ \mathrm{[F/L]} \f$
- `FRDC: real`: Chain / seafloor friction coefficient \f$ \mathrm{[1]} \f$
- `LCAB: real, default: 0`: Cable length \f$ \mathrm{[L]} \f$
- `WCAB: real, default: 0`: Cable weight \f$ \mathrm{[F/L]} \f$
...
- `TMAX: real, default: 0`: Maximum mean tension \f$ \mathrm{[F]} \f$
...
- `EAF(1): real`: Tension corresponding to strain `ELONG(1)` \f$ \mathrm{[F]} \f$
- `ELONG(1): real`: Strain \f$ \mathrm{[\%]} \f$
...
The linear material curve used in dynamic analysis is given by \f$ DSCA + DSCB
\cdot TMEAN \f$, where `TMEAN` is the mean tension of the segment, and by
...
- `GRLEV: real`: Z coordinate of level given in global coordinate system \f$ \mathrm{[L]} \f$
- `GRTH: real`: Growth thickness \f$ \mathrm{[L]} \f$
- `GRDENS: real`: Growth density at this level \f$\left[ \mathrm{M/L^3} \right]\f$
...
- `WDEPTH: real`: Water depth \f$ \mathrm{[L]} \f$ 
...
- `AIRDEN: real > 0`: Air density \f$ \mathrm{ \left[M/L^3 \right]} \f$
- `WATDEN: real > 0`: Water density \f$ \mathrm{ \left[M/L^3 \right]} \f$
- `WAKIVI: real, default: ` \f$ 1.188 \times 10^{-6}\f$: Kinematic viscosity of water \f$ \mathrm{ \left[L^2/T \right]} \f$
- `AIRKIVI: real, default: ` \f$ 1.516 \times 10^{-5}\f$: Kinematic viscosity of air \f$ \mathrm{ \left[L^2/T \right]} \f$

Typical values of `AIRDEN` and `WATDEN`  are
\f$\mathrm{\left[AIRDEN = 1.3 kg/m^3 \right] }\f$
and
\f$\mathrm{\left[WAYDEN = 1025 kg/m^3  \right] }\f$
...
- `SIWAHE: real`: Significant wave-height, \f$ \mathrm{H_S} \f$  \f$ \mathrm{[L]} \f$
- `AVWAPE: real > 0`: Zero-crossing wave-period, \f$ \mathrm{T_Z } \f$  \f$ \mathrm{[T]} \f$

. 
- \f$ \mathrm{S_{\eta}(\omega) = A \omega^{-5} exp \left[-^B/\omega^4 \right]; 0 < \omega < \infty  } \f$
- \f$ \mathrm{A = 124.2 H_S^2/T_Z^4} \f$
- \f$ \mathrm{B = 496/T_Z^4} \f$

The relation between peak period, \f$ \mathrm{T_p} \f$ and zero-crossing period is \f$ \mathrm{T_Z \approx T_p/1.408} \f$
...
- `SIWAHE: real > 0`: Significant wave-height, \f$ \mathrm{H_S} \f$  \f$ \mathrm{[L]} \f$

. 
- \f$ \mathrm{S_{\eta}(\omega) = A \omega^{-5} exp \left[-^B/\omega^4 \right]; 0 < \omega < \infty  } \f$
- \f$ \mathrm{A = 0.0081 g^2} \f$
- \f$ \mathrm{B = 3.11/H_S^2} \f$
...
- `PEAKFR: real > 0`: Peak frequency (wp) \f$ \mathrm{[radians/T]} \f$
...
. 
- \f$ \mathrm{S_{\eta}(\omega) = \alpha g^2 \omega^{-5} exp \left( - \beta \left( \frac{\omega_p}{\omega} \right)^4  \right) \times \gamma^{exp \left( -  \frac{(\omega - \omega_p)^2}{2 \sigma ^2 \omega^2_p}  \right)} } \f$ 
- \f$ \mathrm{\alpha = 1.2905 H_S^2/T_Z^4} \f$
- \f$ \mathrm{\beta = 1.25} \f$ for North Sea conditions
- \f$ \mathrm{\gamma = } \f$
\f$ \begin{cases}    
    \mathrm{1.0;} \quad \mathrm{T_p >= 5 \sqrt{H_S} } \\
    \mathrm{exp(5.75-1.15T_p/ \sqrt{H_S}) } \\
    \mathrm{5.0;} \quad \mathrm{T_p < 3.6 \sqrt{H_S} }
\end{cases} \f$
 - \f$ \mathrm{\sigma = } \f$
 \f$ \begin{cases}
 \mathrm{\sigma_a = 0.07} \quad  \mathrm{for} \quad  \omega <= \omega_p \\
 \mathrm{\sigma_b = 0.09} \quad  \mathrm{for} \quad  \omega <= \omega_p  
\end{cases} \f$
- \f$ \mathrm{ \omega_p = \frac{2\pi}{T_p} } \f$
- \f$ \mathrm{ \frac{T_p}{T_Z} = 1.407(1-0.287 ln \gamma)^{1/4} } \f$
...
- `SPEC1: real, default: 0.214`: Spectrum parameter, a \f$ \mathrm{[T/rad]} \f$
- `SPEC2: real > 2, default: 0.065`: Spectrum parameter, b \f$ \mathrm{[rad/T]} \f$
- `SPEC3: real, default: 0.26`: Spectrum parameter, d \f$ \mathrm{[rad/T]} \f$
- `SIWAHE: real`: Significant wave height, \f$ \mathrm{H_S}  \f$ \f$ \mathrm{[L]} \f$
- `AVWAPE: real > 0`: Average wave period, \f$ \mathrm{T}  \f$ \f$ \mathrm{[T]} \f$
- `TRUNCL: real, default: 0.0414`: Lower truncation parameter \f$ \mathrm{[radians/T]} \f$
- `TRUNCU: real, default: 10.367`: Upper truncation parameter \f$ \mathrm{[radians/T]} \f$

- \f$ \mathrm{S_{\eta}(\omega) = \alpha H_S^2exp \sqrt{\frac{(\omega- \omega_p)^2}{b(\omega - \omega_p + d) } }  } \f$ for \f$ \mathrm{TRUNCL < \omega < TRUNCU} \f$
...
- `FRQ: real`: Frequency \f$ \mathrm{[radians/T]} \f$
- `DSPDEN: real`: Associated discrete spectral density value \f$ \mathrm{[L^2T]} \f$
...
- `SIWAHE: real`: Significant wave height \f$ \mathrm{[L]} \f$
...
- `FETCH: real`: Fetch \f$ \mathrm{[L]} \f$
- `WISPD: real`: Wind speed \f$ \mathrm{[L/T]} \f$
...
- `SIWAHE: real`: Significant wave height \f$ \mathrm{[L]} \f$
- `SIWAPE: real > 0`: Significant wave period \f$ \mathrm{[T]} \f$
...
- `SIWAHE: real`: Significant wave height \f$ \mathrm{[L]} \f$
- `PEAKPE: real > 0`: Peak period \f$ \mathrm{[T]} \f$
...
\f$ \mathrm{GAMMA = exp\left[5.75-1.15 \times \frac{PEAKPE}{\sqrt{SIWAHE}}\right]  }\f$

\f$ \mathrm{1 <= GAMMA <= 5} \f$
...
\f$ \mathrm{f(\alpha_i) = \frac {[cos(\alpha_i-WADR1)]^{ EXPO1}}{\sum[f(\alpha_j)]}} \f$
where \f$ \mathrm{\alpha_i} \f$ is one of the `IWADR1` short-crested wave
...
\f$ \mathrm{f(\alpha_i) = \frac {[cos(\alpha_i-WADR2)]^{ EXPO2}}{\sum[f(\alpha_j)]}} \f$
where \f$ \mathrm{\alpha_i} \f$ is one of the `IWADR2` short-crested wave
...
- `AMPLIT: real`: Wave amplitude \f$ \mathrm{[L]} \f$
- `PERIOD: real > 0`: Wave period \f$ \mathrm{[T]} \f$
- `WAVDIR: real`: Wave propagation direction from the global X-axis \f$ \mathrm{[deg]} \f$
    - Confer the figure [Location of support vessel coordinate system](@ref Location_of_support_vessel_coordinate_system) 
...
- `CURLEV: real`: Z coordinate of level given in global coordinate system \f$ \mathrm{[L]} \f$
...
- `CURVEL: real`: Current velocity at this level \f$ \mathrm{[L/T]} \f$
...
- `WIDIR: real`: Wind propagation direction in global XY-plane \f$ \mathrm{[deg]} \f$
...
- `UMVEL: real`: Longitudinal wind velocity component \f$ \mathrm{[L/T]} \f$
- `VMVEL: real`: Lateral wind velocity component \f$ \mathrm{[L/T]} \f$
- `WMVEL: real`: Vertical (global Z-axis) wind velocity component \f$ \mathrm{[L/T]} \f$
...
- `ZLEV: real`: Vertical coordinate of profile level \f$ \mathrm{[L]} \f$
...
- `Z0: real`: Z coordinate of the lower edge of the wind field domain \f$ \mathrm{[L]} \f$
...
- `DLWFZ: real`: Domain resolution in the vertical direction \f$ \mathrm{[L]} \f$
...
- `UMVEL: real`: Mean wind velocity along `WIDIR` \f$ \mathrm{[L/T]} \f$
...
- `X0LL: real`: X-coordinate of the lower left corner of the upstream border of the
    wind field domain \f$ \mathrm{[L]} \f$
- `Y0LL: real`: Y-coordinate of the lower left corner of the wind field domain \f$ \mathrm{[L]} \f$
- `Z0LL: real`: Z-coordinate of the lower left corner of the wind field domain\f$ \mathrm{[L]} \f$
...
- `NX: integer`: Number of grid points in X- (longitudinal) direction \f$ \mathrm{[L]} \f$
- `NY: integer`: Number of grid points in Y- (lateral) direction \f$ \mathrm{[L]} \f$
- `NZ: integer`: Number of grid points in Z- (vertical) direction\f$ \mathrm{[L]} \f$
...
- `ZREF: real`: Reference height \f$ \mathrm{[L]} \f$
- `ALPHA: real`: Wind shear exponent \f$ \mathrm{[-]} \f$
...
- `ZREF: real`: Reference height \f$ \mathrm{[L]} \f$
- `Z0: real`: Roughness length \f$ \mathrm{[L]} \f$
...
- `ZG: real`: Vertical position of the support vessel coordinate system. \f$ \mathrm{[L]} \f$
...
- radian/radians or degrees/degrees for freedoms roll, pitch and yaw, giving motion
    ...
    \f$ \mathrm{\gamma_a = k \zeta_a }\f$ where \f$ \mathrm{k} \f$ is the wave number and \f$ \mathrm{\zeta_a} \f$ is
    the wave amplitude.
...
- `WHFTR: real`: Frequency \f$ \mathrm{[rad/T]} \f$
...
- `ROTX0: real, default: 0`: Rotation around X-axis \f$ \mathrm{[deg]}\f$
- `ROTY0: real, default: 0`: Rotation around Y-axis \f$ \mathrm{[deg]}\f$
- `ROTZ0: real, default: 0`: Rotation around global Z-axis \f$ \mathrm{[deg]}\f$
...
~~~
- `XG0: real, default: 0`: Initial global X-coordinate of `SIMO` body node \f$ \mathrm{[L]}\f$
- `YG0: real, default: 0`: Initial global Y-coordinate of `SIMO` body node \f$ \mathrm{[L]}\f$
- `ZG0: real, default: 0`: Initial global Z-coordinate of `SIMO` body node \f$ \mathrm{[L]}\f$
- `ROTX0: real, default: 0`: Initial rotation around X-axis \f$ \mathrm{[deg]}\f$
- `ROTY0: real, default: 0`: Initial rotation around Y-axis \f$ \mathrm{[deg]}\f$
- `ROTZ0: real, default: 0`: Initial rotation around global Z-axis \f$ \mathrm{[deg]}\f$
...
~~~
- `XG: real, default: XG0`: Global X-coordinate of `SIMO` body node at final static equilibrium \f$ \mathrm{[L]}\f$
- `YG: real, default: YG0`: Global Y-coordinate of `SIMO` body node at final static equilibrium \f$ \mathrm{[L]}\f$ 
- `ZG: real, default: ZG0`: Global Z-coordinate of `SIMO` body node at final static equilibrium \f$ \mathrm{[L]}\f$ 
- `ROTX: real, default: ROTX0`: Rotation around X-axis at final static equilibrium \f$ \mathrm{[deg]}\f$
- `ROTY: real, default: ROTY0`: Rotation around Y-axis at final static equilibrium \f$ \mathrm{[deg]}\f$
- `ROTZ: real, default: ROTZ0`: Rotation around global Z-axis at final static equilibrium \f$ \mathrm{[deg]}\f$
...
- `STX: real, default: 0`: Stiffness in global X-direction \f$ \mathrm{[F/L]}\f$
- `STY: real, default: 0`: Stiffness in global Y-direction \f$ \mathrm{[F/L]}\f$
- `STZ: real, default: 0`: Stiffness in global Z-direction \f$ \mathrm{[F/L]}\f$
- `SRX: real, default: 0`: Stiffness around global X-direction \f$ \mathrm{[FL/deg]}\f$
- `SRY: real, default: 0`: Stiffness around global Y-direction \f$ \mathrm{[FL/deg]}\f$
- `SRZ: real, default: 0`: Stiffness around global Z-direction \f$ \mathrm{[FL/deg]}\f$
...
- `AMS: real`: Mass/unit length \f$ \mathrm{[M/L]} \f$
- `AE: real`: External volume per length \f$ \mathrm{ \left[L^2 \right]} \f$
...
- .
- .
- `ELONG(N): real`: Relative elongation \f$ \mathrm{[1]} \f$
...
- `SN: real`: Solidity ratio \f$ \mathrm{[1]} \f$
- `WIDTH1: real`: Net width at segment end 1 \f$ \mathrm{[L]} \f$
- `WIDTH2: real`: Net width at segment end 2 \f$ \mathrm{[L]} \f$
...
- `AMX: real`: Added mass per length, tangential direction \f$ \mathrm{[M/L]} \f$
- `AMY: real`: Added mass per length, normal direction \f$ \mathrm{[M/L]} \f$
...
- `TB: real`: Tension capacity \f$ \mathrm{[F]} \f$
- `YCURMX: real`: Maximum curvature \f$ \mathrm{[I/L]} \f$
...
\f$ \mathrm{ \omega_{f.k} = \alpha \omega_{f.k-1} + (1 - \alpha) \omega_K }\f$

where

\f$ \mathrm{ \alpha = exp((-\Delta t)/(TC))}\f$

where \f$ \mathrm{ \Delta t}\f$ is the discrete time
step, \f$ \mathrm{ TC}\f$ is the filter time constant,
\f$ \mathrm{ \alpha}\f$ is the low-pass filter
coefficient \f$ \mathrm{ \omega_f}\f$ is low pass
filtered generator speed and \f$ \mathrm{k }\f$ indicates the time step. The relation
between the filter time constant and the cut off (corner) frequency 
\f$ \mathrm{ f_C}\f$ is given by:

\f$ \mathrm{ TC = \frac{1}{2 \pi f_C}}\f$
...
\f$ \mathrm{ (K(s + a))/s}\f$ where \f$ \mathrm{K }\f$ and \f$ \mathrm{a }\f$ are the
proportional gain and the integrator gain. The corresponding and simple
regulator algorithm is given by

\f$ \mathrm{R(t + \Delta t) = R(t) + \Delta \omega \Delta t }\f$

\f$ \mathrm{ \theta = K_P \Delta \omega + a K_P R (t + \Delta t) = K_P \Delta \omega + K_I R (t + \Delta t)}\f$

where \f$ \mathrm{\Delta t }\f$ is the regulator time
step, \f$ \mathrm{ \Delta \omega}\f$ is the rotor speed
error, i.e. the difference between filtered rotor speed and rated rotor
speed. \f$ \mathrm{ R}\f$ is accumulated time integrated speed error which is set to zero
for filtered generator speed less than rated generator speed.
\f$ \mathrm{ \theta}\f$ is the instructed/required
...
- `GNS_RATE: real > 0`: Rated generator speed \f$ \mathrm{[rad/T]}\f$
- `TRQ_RATE: real > 0`: Rated generator torque \f$ \mathrm{[FL]}\f$
- `RGN3MP: real`: Minimum pitch angle for which electrical torque versus generator
speed will stay in region 3 \f$ \mathrm{[deg]}\f$
...
- `RGN15SP: real > 0`: Transitional generator speed between region 1 and 1 1⁄2.
    Start speed for extracting power. \f$ \mathrm{[rad/T]}\f$
- `RGN20SP: real > RGN15SP`: Transitional generator speed between region 1 1⁄2 and 2. \f$ \mathrm{[rad/T]}\f$
- `RGN25SP: real > RGN20SP`: Transitional generator speed between region 2 and 2 1⁄2 \f$ \mathrm{[rad/T]}\f$
- `RGN30SP: real > RGN25SP`: Transitional generator speed between region 2 1⁄2 and 3 \f$ \mathrm{[rad/T]}\f$
- `TRQRGN2: real > 0`: Generator torque constant in region 2\f$ \mathrm{\left[ FL/(rad/T)^2 \right] }\f$
...
- `TRQ_MAXRAT: real > 0`: Maximum torque rate \f$ \mathrm{[FL/T]}\f$
- `TRQ_MAX: real > 0`: Maximum electrical torque \f$ \mathrm{[FL]}\f$
...
- `PC_MINPIT: real`: Minimum pitch setting in pitch controller \f$ \mathrm{[deg]}\f$
- `PC_MAXPIT: real`: Maximum pitch setting in pitch controller \f$ \mathrm{[deg]}\f$
- `PC_MAXRAT: real`: Maximum pitch rate \f$ \mathrm{[deg/T]}\f$
...
- `G_SHEDULE: character`: Gain scheduling; Default or Tabulated
    - `= D`: Default
    - `= T`: Tabulated 
- `TC: real`: Time constant for first order low pass filter, \f$ \mathrm{TC = 1/\omega}\f$ \f$ \mathrm{[s/rad]}\f$
...
- `BPITCH: real`: Blade pitch angle \f$ \mathrm{[deg]}\f$
...
- `DTSAMP: real > 0`: Controller sample interval \f$ \mathrm{[T]}\f$
...
- omega: Rotor velocity \f$ \mathrm{[rad/T]}\f$. May be extracted with the Java method getOmega().

- pitch angles: Blade pitch angles for all blades \f$ \mathrm{[rad]}\f$. May be extracted with the Java method getPitchAngle(i), i = 0, ..., nblades-1.

- position: (x,y,z,rx,ry,rz) dynamic displacement \f$ \mathrm{[L]}\f$
and rotation \f$ \mathrm{[rad]}\f$ from final static position /
...
- velocity: (vx,vy,vz,vrx,vry,vrz) velocity \f$ \mathrm{[L/T]}\f$ and
angular velocity \f$ \mathrm{[rad/T]}\f$ for the node at the second
...
- acceleration: (ax,ay,az,arx,ary,arz) acceleration \f$
\mathrm{[L/T^2]}\f$ and angular acceleration \f$ \mathrm{[rad/T^2]}\f$
...
- hub wind: (wix,wiy,wiz) hub wind velocity \f$ \mathrm{[L/T]}\f$,
...
blades \f$ \mathrm{[FL]}\f$. May be extracted with the Java method 
getBladeRootBMX(i), i = 0, ..., nblades-1.

- blade root bending moment around local element y-axis for all blades
\f$ \mathrm{[FL]}\f$. May be extracted with the Java method 
...
for each node: 3 displacements \f$ \mathrm{[L]}\f$ and 3 rotations \f$
\mathrm{[rad]}\f$ relative to the stress-free configuration, 6
velocities \f$ \mathrm{[L/T]}\f$ or \f$ \mathrm{[rad/T]}\f$ and 6
accelerations \f$ \mathrm{[L/T^2]}\f$ or \f$
\mathrm{[rad/T^2]}\f$. The rotations (R1,R2,R3) are taken in the order
...
values for each element: effective tension \f$ \mathrm{[F]}\f$,
torsional moment \f$ \mathrm{[FL]}\f$, My-moment end 1 and 2 \f$
\mathrm{[FL]}\f$, Mz-moment end 1 and 2 \f$ \mathrm{[FL]}\f$, Qy-shear
force end 1 and 2 \f$ \mathrm{[F]}\f$, Qz-shear force end 1 and 2 \f$
\mathrm{[F]}\f$. Local element system. May be extracted with the Java
...
- Torque request: Actuator torque to apply on rotor axis \f$
\mathrm{[ML^2/T^2]}\f$

- Pitch angle request for all blades: Controller pitch angle \f$
  \mathrm{[rad]}\f$

- Gear shaft omega: Gear shaft rotor speed \f$ \mathrm{[rad/T]}\f$
Used for presentation only.

- Generated power: Generated electrical power \f$ \mathrm{[kW]}\f$
Used for presentation only.
...
and the line from the tip of the wedge, low (negative) angles of attack  \f$ \mathrm{[deg]}\f$
- `ATAIL2: real`: as ATAIL1, but for high (positive) angles of attack \f$ \mathrm{[deg]}\f$
- `ANOSE1: real`: nose angle between a line perpendicular to the flow 
and the line from the tip of the wedge, low (negative) angles of attack \f$ \mathrm{[deg]}\f$
- `ANOSE2: real`: as ANOSE1, but for high (positive) angles of attack \f$ \mathrm{[deg]}\f$
- `RNOSEC: real`: ratio of the nose radius to the chord of the airfoil \f$ \mathrm{[1]}\f$
...
- `RE: real`: Reynolds number for this block \f$ \mathrm{[1]}\f$
...
- `AOA0: real`: the angle of attack where there is zero lift, upcrossing \f$ \mathrm{[deg]}\f$
- `DCLDA1: real`: maximum slope of the lift curve in the 
linear region above AOA0 \f$ \mathrm{[1/deg]}\f$
- `DCLDA2: real`: maximum slope of the lift curve in the 
linear region below AOA0 \f$ \mathrm{[1/deg]}\f$
- `AOAFS1: real`: angle of attack corresponding to full separation above AOA0 \f$ \mathrm{[deg]}\f$
- `AOAFS2: real`: angle of attack corresponding to full separation below AOA0 \f$ \mathrm{[deg]}\f$
...
- `AOA: real`: angle of attack \f$ \mathrm{[deg]}\f$
- `CL: real`: non-dimensional lift coefficient \f$ \mathrm{[1]}\f$
- `CD: real`: non-dimensional drag coefficient \f$ \mathrm{[1]}\f$
- `CM: real`: non-dimensional moment coefficient \f$ \mathrm{[1]}\f$
